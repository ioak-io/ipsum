...
import {
  IPSUM_PREF_ADDBILL_ANOTHER,
  IPSUM_PREF_ADDBILL_DATE,
...
const EditBillPage = (props: Props) => {
  ...
  const getEmptyBill = (): ReceiptModel => {
    return {
      ...EMPTY_BILL,
      billDate:
        sessionStorage.getItem(IPSUM_PREF_ADDBILL_DATE) ||
        EMPTY_BILL.billDate,
    };
  };
  ...
  useEffect(() => {
    if (sessionStorage.getItem(IPSUM_PREF_ADDBILL_ANOTHER)) {
      setAddAnother(
        sessionStorage.getItem(IPSUM_PREF_ADDBILL_ANOTHER) === 'true'
      );
    }
  }, []);
