...
const ProtectedRouteApp = (props: Props) => {
    ...
    const authenticate = async (type: string, redirect = true) => {
        ...
        const accessToken = getSessionValue(`ipsum-access_token`);
        const refreshToken = getSessionValue(`ipsum-refresh_token`);
        if (accessToken && refreshToken) {
            httpPost(
                ...
                .then((response: any) => {
                    if (response.status === 200) {
                        ...
                        if (response.data?.access_token) {
                            ...
                            setSessionValue(`ipsum-access_token`, newAccessToken);
                        }
                        dispatch(
                            addAuth({
                                isAuth: true,
                ...
                .catch((error: any) => {
                    removeSessionValue(`ipsum-access_token`);
                    removeSessionValue(`ipsum-refresh_token`);
